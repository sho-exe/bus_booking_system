...
public class UpdateProfileServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (isSuccess) {
            // Update the session so the new name/email shows up immediately on the top right
            HttpSession session = request.getSession();
            session.setAttribute("driver", updatedDriver);
            
            // Redirect back to the portal page with a success flag
            response.sendRedirect("DriverPortal?update=success");
        } else {
            // Redirect back with an error flag
            response.sendRedirect("DriverPortal?update=error");
        }
